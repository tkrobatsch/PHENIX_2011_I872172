...
namespace Rivet
{
	...
	class PHENIX_2011_I872172 : public HeavyIonAnalysis
	{
	...
	private:
		static const int N_CENT_TYPES = 2; // number of centrality bins that you need to worry about, this case two, 10-30, 50-80
		static const int CENT_TYPE_EDGES[N_CENT_TYPES][2] = { {0,20},{20,60} }; // 10-30, 50-80
	...
	public:
		/// Constructor
		PHENIX_2011_I872172() :HeavyIonAnalysis("PHENIX_2011_I872172") {}
		...
		void init()
		{
			...
			const int pidPI0; // The PID for Pi0
			...
			//Cuts cutTrigger = Cuts::abseta < .35 && ((Cuts::pid == pidPI0 && Cuts::pt > 13.0 * GeV && Cuts::pt < 20.0 * GeV) || (Cuts::pid == pidGAMMA && Cuts::pt > 8.0 * GeV && Cuts::pt < 20.0 * GeV))
			Cut cutTrigger = Cuts::abseta < 1 && (Cuts::pid == pidPI0 && (Cuts::pT > 4 * GeV && Cuts::pT < 7 * GeV));
			...
			Cuts cutAssoc = Cuts::abseta < 1 && Cuts::pt > 3 * GeV && Cuts::pt < 7 * GeV;
			...
			foreach(const Particle & partTrigger, tracksTrigger) {
				...
				foreach(const Particle & partAssoc, tracksAssoc) {
					...
					if (partAssoc.pt() < partTrigger.pt()) {
						...
						while (deltaPhi < 0) deltaPhi += M_PI;
